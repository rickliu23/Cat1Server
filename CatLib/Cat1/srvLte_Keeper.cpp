...
bool clsLteKeeperIf::MsgPop(LTE_AT_CMD_TYPE &cmdType, uint8_t *msg, uint32_t lenIn, uint32_t &lenOut, int32_t &timeout_ms)
{
...
void clsLteKeeperIf::WaitAndFindNextMsg(uint32_t time_ms)
{
    ...
    {
        m_msgStatus.nowCmd = LTE_AT_CMD_AT;

        isFind = Lte_AT_Assemble_Basic(LTE_AT_CMD_AT, LTE_AT_READ, 0, (char *)m_msgStatus.msgBuf, sizeof(m_msgStatus.msgBuf), (int *)&m_msgStatus.msgLen);

        ptr = &m_moduleStatus.isModuleReachable;
    }
    else if(m_moduleStatus.isEchoOn.flag == false)
    {
        m_msgStatus.nowCmd = LTE_AT_CMD_ECHO;

        isFind = Lte_AT_Assemble_Basic(LTE_AT_CMD_ECHO, LTE_AT_WRITE, 1, (char *)m_msgStatus.msgBuf, sizeof(m_msgStatus.msgBuf), (int *)&m_msgStatus.msgLen);

        ptr = &m_moduleStatus.isEchoOn;
    }
    else if(m_moduleStatus.isSimExist.flag == false)
    {
        m_msgStatus.nowCmd = LTE_AT_CMD_SIM_PIN;

        isFind = Lte_AT_Assemble_Basic(LTE_AT_CMD_SIM_PIN, LTE_AT_READ, 0, (char *)m_msgStatus.msgBuf, sizeof(m_msgStatus.msgBuf), (int *)&m_msgStatus.msgLen);

        ptr = &m_moduleStatus.isSimExist;
    }
    else if(m_moduleStatus.isCSQ.flag == false)
    {
        m_msgStatus.nowCmd = LTE_AT_CMD_CSQ;

        isFind = Lte_AT_Assemble_Basic(LTE_AT_CMD_CSQ, LTE_AT_READ, 0, (char *)m_msgStatus.msgBuf, sizeof(m_msgStatus.msgBuf), (int *)&m_msgStatus.msgLen);

        ptr = &m_moduleStatus.isCSQ;
    }
    else if(m_moduleStatus.isReg.flag == false)
    {
        m_msgStatus.nowCmd = LTE_AT_CMD_NET_REG;

        isFind = Lte_AT_Assemble_Basic(LTE_AT_CMD_NET_REG, LTE_AT_READ, 0, (char *)m_msgStatus.msgBuf, sizeof(m_msgStatus.msgBuf), (int *)&m_msgStatus.msgLen);
...
bool clsLteKeeperIf::AT_RegProcess(uint8_t *msg, uint32_t lenIn)
{
    ...
    if(stat == 1 || stat == 5) // 1 = 注册，本地   5 = 注册，漫游
    {
        return true;
    }
